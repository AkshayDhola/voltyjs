...
const usePageTransition = ({ ref, transitionStyle, color, gridLength }) => {
  ...
  useEffect(() => {
    ...
    const removeElements = () => {
      document.querySelectorAll(".loader-container, .transition-container").forEach((el) => el.remove());
    };
    ...
    switch (transitionStyle) {
      case 1: {
        const loaderContainer = createLoaderContainer(gridLength);
        const bgElements = gsap.utils.toArray(".loader-container div");
        gsap.set(bgElements, { clipPath: "polygon(-1% -1%, 101% -1%, 101% 101%, -1% 101%)" });
        gsap.timeline()
          .to(bgElements, { clipPath: "polygon(0% 0%, 98% 0%, 98% 100%, 0% 100%)", duration: 1.5, ease: "power4", delay: 0.5 })
          .to(bgElements, { clipPath: "polygon(0% 0%, 0% 0%, 0% 100%, 0% 100%)", duration: 1.5, ease: "power4", onComplete: removeElements });
        break;
      }

      case 2: {
        const loaderContainer = createLoaderContainer(4);
        const bgElements = gsap.utils.toArray(".loader-container div");
        gsap.set(bgElements, { yPercent: 0 });
        gsap.to(bgElements, { yPercent: 100, stagger: 0.4, duration: 1.7, onComplete: removeElements });
        break;
      }
      ...
      case 3: {
        ...
        gsap.timeline()
          .to(path, { duration: 0.8, attr: { d: "M0 502S175 272 500 272s500 230 500 230V0H0Z" }, ease: "power2.in" })
          .to(path, { duration: 0.8, attr: { d: "M0 2S175 1 500 1s500 1 500 1V0H0Z" }, ease: "power2.out", onComplete: removeElements });
        break;
      }
      ...
      case 4: {
        ...
        gsap.to(row1.children, { duration: 1, scaleY: 0, stagger: 0.2, transformOrigin: "top", ease: "power2.out" },["hello"]);
        gsap.to(row2.children, { duration: 1, scaleY: 0, stagger: 0.2, transformOrigin: "bottom", ease: "power2.out", delay: 0.5, onComplete: removeElements },["hello"]);
        break;
      }      
      default:
...
export const PageTransition=({ children, transitionStyle = 1, color = "black", gridLength=7 }) => {
